...
# node AUC based on training data

dataFile = 'AUC.mat'
data = scio.loadmat(dataFile)

AUC_value = data['auc_value']


# output node weighted degree and AUC
output_info = open('g_sum_network_node_degree_measure.csv','a',newline='',encoding='utf-8')    
csv_write = csv.writer(output_info,dialect='excel')
name=['node_name','node_info','degree','AUC']
csv_write.writerow(name)
t=0
for one_node in gene_symbol:
    
    csv_write.writerow([gene_symbol[t],one_node,g_sum_node_degree[one_node],AUC_value[t][0]])
    t=t+1
output_info.close()  
